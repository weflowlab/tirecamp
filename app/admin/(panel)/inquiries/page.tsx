...
type Props = { searchParams: Promise<{ status?: string; q?: string; page?: string; open?: string; range?: string; from?: string; to?: string }> };

/**
 * 문의 관리 (/admin/inquiries?range=…&status=new|done&q=검색어&page=N&open=id)
 * - 기간(기본 전체) · 새로고침 · 엑셀 다운로드 → 상태 칩 + 검색 → 목록(펼치면 내용/메모/처리 버튼) → 페이지 번호
 */
export default async function AdminInquiries({ searchParams }: Props) {
  ...
  const firstDay = all.length ? all[all.length - 1].date.replace(/\./g, "-") : today;
  const period = resolvePeriod(sp, today, "all", firstDay);

  const inRange = all.filter((i) => inPeriod(i.date, period));
  const filtered = inRange.filter((i) => (!status || i.status === status) && (!q || [i.name, i.phone, i.car, i.content, i.memo ?? ""].some((v) => v.includes(q))));
  ...
  const query = (over: Record<string, string | number>) => {
    ...
    const merged: Record<string, string | number> = { ...pp, status, q, page: 1, ...over };
    for (const k of ["range", "from", "to", "status", "q"]) if (merged[k]) p.set(k, String(merged[k]));
    if (merged.page && Number(merged.page) > 1) p.set("page", String(merged.page));
    const s = p.toString();
    return `/admin/inquiries${s ? `?${s}` : ""}`;
  };
  const count = (s: "" | "new" | "done") => inRange.filter((i) => !s || i.status === s).length;
  const exportHref = `/api/admin/inquiries/export?${new URLSearchParams({ ...pp, ...(status ? { status } : {}), ...(q ? { q } : {}) }).toString()}`;

  return (
    <div>
      <PageHead eyebrow="Inquiries" title="문의 관리" desc="문의하기 폼으로 접수된 문의입니다. 처리 후 완료로 바꾸고, 메모를 남겨 두세요." />

      <PeriodBar basePath="/admin/inquiries" period={period} today={today} keep={{ status, q }} exportHref={exportHref} />
