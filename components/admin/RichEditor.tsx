  ...
  return (
    <div className="border border-line bg-white focus-within:border-ink">
      ...
      <div className="flex items-center gap-[14px] border-b border-line px-[12px] py-[8px]">
        <button type="button" onMouseDown={(e) => e.preventDefault()} onClick={() => exec("bold")} className="h-[26px] w-[26px] border border-line text-[14px] font-bold text-ink hover:bg-surface" title="굵게">
          B
        </button>
        <button type="button" onMouseDown={(e) => e.preventDefault()} onClick={() => exec("underline")} className="h-[26px] w-[26px] border border-line text-[14px] text-ink underline hover:bg-surface" title="밑줄">
          U
        </button>
        <button type="button" onMouseDown={(e) => e.preventDefault()} onClick={() => exec("insertHorizontalRule")} className="h-[26px] border border-line px-[8px] text-[13px] text-ink hover:bg-surface" title="구분선">
          구분선
        </button>
      ...
      <div
        ...
        className="rich-editor px-[16px] py-[14px] text-[15px] leading-[26px] text-graphite outline-none [&_img]:my-[8px] [&_img]:block [&_img]:max-w-full [&_p]:min-h-[26px] [&_hr]:my-[14px] [&_hr]:border-line [&_b]:text-ink [&_strong]:text-ink"
        style={{ minHeight }}
      />
    </div>
  );
