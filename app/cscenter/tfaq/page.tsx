...
export default async function FaqPage() {
  ...
  return (
    <div className="w-full font-sans">
        ...
        {FAQS.map((f, i) => (
          <li key={i} className="border-b border-line">
            <details className="group">
              <summary className="flex cursor-pointer list-none items-center gap-[20px] py-[18px] text-[16px] font-medium text-ink hover:text-graphite [&::-webkit-details-marker]:hidden max-pc:gap-[14px] max-pc:text-[15px]">
                ...
                <span className="shrink-0 text-[19px] font-light leading-none text-faint transition-transform group-open:rotate-45">+</span>
              </summary>
              <div className="pb-[22px] pl-[40px] pr-[30px] text-[15px] leading-[25px] text-graphite max-pc:pl-[34px] max-pc:pr-0">
                <p className="whitespace-pre-line text-[15px] text-graphite">{f.a}</p>
              </div>
            </details>
          </li>
        ))}
      ...
      <p className="mt-[24px] text-[14px] leading-[22px] text-muted max-pc:text-center">
        찾는 답이 없으신가요?
